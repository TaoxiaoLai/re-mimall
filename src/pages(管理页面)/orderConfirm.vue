      ...
      <div class="container">
        <div class="order-box">
          ...
          <div class="btn-group">
            ...
            <a href="javascript:;" class="btn btn-large" @click="orderConfirm">去结算</a>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      showDelModal: false, //删除弹框显示控制
      showEditModal: false, //新增和编辑地址弹框控制
  ...
  methods: {
    ...
    submitAddress() {
      ...
      if(userAction == 0) {
        url = '/shippings', method = 'post'
      }else if(userAction == 1) {
        url = `/shippings/${checkedItem.id}`, method = 'put'
      }else {
        url = `/shippings/${checkedItem.id}`, method = 'delete'
      }
